...
export default async function ArtistDetail({ params }: { params: Promise<Params> }) {
  ...
  // Find chapters and events associated with this artist
      ...
      {artist.streetLibrary && (
        <div className="mt-8 mb-8">
          ...
          <div className="mb-8 p-6 bg-gray-50 rounded-lg border-l-4 border-gray-400">
            ...
            <p className="text-gray-700 leading-relaxed japanese-text">{artist.streetLibrary.concept}</p>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {artist.streetLibrary.images.map((image, index) => (
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="p-5 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <p className="text-sm text-gray-700 leading-relaxed japanese-text">{artist.streetLibrary.mechanism.write}</p>
              </div>
              <div className="p-5 bg-gray-100 rounded-lg border border-gray-300">
                ...
                <p className="text-sm text-gray-700 leading-relaxed japanese-text">{artist.streetLibrary.mechanism.read}</p>
              </div>
              <div className="p-5 bg-white rounded-lg border border-gray-400">
                ...
                <p className="text-sm text-gray-700 leading-relaxed japanese-text">{artist.streetLibrary.mechanism.weave}</p>
              </div>
            </div>
          </div>
          ...
          {/* Features */}
          <div className="mb-8">
            <h3 className="text-lg font-semibold mb-4 japanese-text">コンセプトの特徴</h3>
            <div className="space-y-3">
              {artist.streetLibrary.features.map((feature, index) => (
                <div key={index} className="flex items-start p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <span className="w-6 h-6 bg-gray-600 text-white rounded-full flex items-center justify-center text-sm mr-3 mt-0.5 flex-shrink-0">
                    {index + 1}
                  </span>
                  <p className="text-sm text-gray-800 leading-relaxed japanese-text">{feature}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-6 bg-gray-100 rounded-lg border-l-4 border-gray-500">
            ...
            <p className="text-gray-700 leading-relaxed japanese-text italic">{artist.streetLibrary.message}</p>
          </div>
        </div>
      )}
